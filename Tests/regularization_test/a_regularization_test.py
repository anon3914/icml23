...
parser.add_argument('--method', type = str,default='census',
...
if args.method[:6] == 'census':
    ...
    if args.kernel == 'WEG' and args.explainer in gpec_explainer_list:
        if args.nn_retrain == 1:
            tmp = decision_boundary.gpec_db_nn(model, tr_data_im, tr_loader, nn_params, save = True, save_path = curdir + '/db.pkl')
        else:
            try:
...
if args.explainer == 'kernelshap' or args.explainer == 'lime' or args.explainer == 'deepshap':
    ...
    save_path = './Files/Results/regularization_test_icml/%s.pkl' % filename
    print(save_path)
